...
def dashboard():
    ...
    print(brokenMachine)
    cursor.execute("""SELECT m.name, pm.time, pm.pm_type, pm.job_description
...
def machine_detail(machine_id):
    ...
    qr=qrcode.make(f"https://71ab-2404-c0-2130-00-4750-f3f7.ngrok-free.app/machine/"+machine_id)
    buffer = io.BytesIO()
    qr.save(buffer, "PNG")
...
def machineSparepart(machine_id):
    ...
    return render_template("machine_sparepart.html", spareparts=sperpart, machineId=machine_id)
...
def sparepart_edit(sparepart_id):
    ...
    if request.method == "POST":
        ...
        mysql.connection.commit()
    cursor.close()
    return render_template('sparepart_edit.html', sparepart=sparepart)
...
def sparepart_add():
    ...
    if request.method == 'POST':
        ...
        mysql.connection.commit()
        cursor.close()
    return render_template('sparepart_add.html')
...
def historyPM():
    ...
    cursor.execute("""SELECT pm.time, pm.pm_type, m.name, m.line, m.id, pm.id
                   ...
                   JOIN machine m ON pm.machine_id=m.id
                   WHERE time<=%s """, (today,))
    historyPm = cursor.fetchall()
    print(role[0])
    cursor.close()
    return render_template("pm_history.html", pm=historyPm, role=role)
...
def workorder():
    ...
    print(allMesin)
    cursor.close()
    return render_template("workorder.html",allMesin=allMesin, form=form, workorder=workOrder)
