...
def machine_detail(machine_id):
    ...
    qr=qrcode.make(f"https://17dd-118-137-165-189.ngrok-free.app/machine/"+machine_id)
    buffer = io.BytesIO()
    qr.save(buffer, "PNG")
...
def pm():
    ...
    print(role[0])
    cursor.close()
    return render_template("rencanaPreventiveMaintenance.html", pm=pmAll, role=role)
...
def historyPM():
    ...
    cursor.execute("""SELECT pm.time, pm.pm_type, m.name, m.line, m.id, pm.id
                   ...
                   JOIN machine m ON pm.machine_id=m.id
                   WHERE time<=%s 
                   order by pm.time DESC """, (today,))
    historyPm = cursor.fetchall()
    print(role[0])
    cursor.close()
    return render_template("historyPreventiveMaintenance.html", pm=historyPm, role=role)
...
def historyCorrectiveMaintenance():
    ...
    historyCm = cursor.fetchall()
    print(role[0])
    print(historyCm)
    cursor.close()
    return render_template("historyCorrectiveMaintenance.html", pm=historyCm, role=role)
