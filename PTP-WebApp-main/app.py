...
def pm():
    ...
    for m in pmAll:
        kategori = m[6]
        if kategori not in pm:
            pm[kategori] = []
        pm[kategori].append(m)
    cursor.close()
    return render_template("rencanaPreventiveMaintenance.html", pm=pm, role=role)
...
def historyPM():
    ...
    for p in pm:
        kategori = p[6]
        if kategori not in historyPm:
            historyPm[kategori] = []
        historyPm[kategori].append(p)
