import base64
import bcrypt, os, time, zipfile, io, qrcode
from datetime import date
from flask import Flask, render_template, redirect, send_from_directory, url_for, flash, session, request, send_file
...
def uploadFile(file, machine_id):
    ...
    if file:
        ...
        cursor=mysql.connection.cursor()
        try:
            cursor.execute("INSERT INTO backupfile (nama, file, id_mesin) VALUES (%s, %s, %s)", (zip_filename, zip_data, machine_id))
...
def machine():
    ...
    cursor.close()
    print(mesin)
    return render_template("machine.html", mesin=mesin, kategori_mesin=kategori_mesin, line_list=line_list)
...
def machine_detail(machine_id):
    ...
    # qr=qrcode.make(f"https://3706-210-79-138-82.ngrok-free.app/machine/"+machine_id)
    qr=qrcode.make(machine_id)
    buffer = io.BytesIO()
    qr.save(buffer, "PNG")
...
def sparepart(machine_id):
    cursor = mysql.connection.cursor()
    cursor.execute("""SELECT s.id, s.nama, s.price, ms.qty
                   FROM sparepart s 
                   JOIN machine_sparepart ms ON ms.id_sparepart = s.id
                   WHERE ms.id_mesin=%s""", (machine_id,))
    sparepart = cursor.fetchall()
    print(sparepart)
    
    return render_template("sparepart.html", spareparts=sparepart)
...
def pm():
    ...
    pm = cursor.fetchall()
    cursor.close()
    return render_template("pm.html", pm=pm, role=role)
...
def editPM(pm_id):
    ...
    cursor.execute("""SELECT ps.sparepart_id, s.name, ps.qty
                   FROM pm_sparepart ps JOIN sparepart s ON ps.id_sparepart=s.id
                   WHERE id_pm=%s""", (pm_id,))
    usedSpareparts = cursor.fetchall()
...
def finishWo(wo_id):
    ...
    if form.validate_on_submit():
        ...
        cursor.execute("""UPDATE wo SET 
                    start_time=%s, finish_time=%s, user_id=%s, status=%s
